...
import type { CarCreateInput, CarUpdateInput, CarRespondInput, CarVerifyInput, CarListQuery } from "@/lib/validations/car";
import type { CarStatus, CarSourceType, VerificationResult } from "@/generated/prisma/client";
import { sendCarIssuedEmail, sendCarReminderEmail, sendCarRespondedEmail, sendCarVerifyPassEmail, sendCarVerify2NotifyEmail, sendCarReCarEmail } from "@/services/carEmailService";
...
export class CarService {
  ...
  async issueCar(id: string, actorId: string): Promise<CarDetail> {
    ...
    if (emailTarget) {
      ...
      await this.carRepo.createNotificationLog({ carMasterId: id, type: "ISSUED", recipient: emailTarget });
    }
  ...
  async respondToCar(
    ...
    const recipients = [mrEmail, qmsEmail].filter(Boolean) as string[];
    if (recipients.length > 0) {
      sendCarRespondedEmail({ carId: id, carNo: car.carNo, recipients }).catch((err) =>
        logger.error("[CarService.respondToCar] Email failed", err)
      );
      for (const r of recipients) {
        await this.carRepo.createNotificationLog({ carMasterId: id, type: "RESPONSE_RECEIVED", recipient: r });
      }
    }
  ...
  async verifyCar(
    ...
    const expectedStatus: CarStatus = input.round === 1 ? "RESPONDED" : "VERIFY_2";
    ...
    // Email notifications
    if (input.result === "PASSED" && actionTokenValue) {
      const mrEmail = await this.configRepo.findValueByKey("CURRENT_MR_EMAIL");
      if (mrEmail) {
        ...
        await this.carRepo.createNotificationLog({ carMasterId: id, type: "VERIFY_1_PASS", recipient: mrEmail });
      }
      ...
      if (deptEmail) {
        ...
        await this.carRepo.createNotificationLog({ carMasterId: id, type: "VERIFY_2_NOTIFY", recipient: deptEmail });
      }
    }
  ...
  async createReCar(originalId: string, actorId: string): Promise<{ newCarId: string; newCarNo: string }> {
    ...
    if (emailTarget) {
      ...
      await this.carRepo.createNotificationLog({ carMasterId: newId, type: "RE_CAR", recipient: emailTarget });
    }
