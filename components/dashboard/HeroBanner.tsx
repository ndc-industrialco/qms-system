...
export default function HeroBanner({ announcements }: { announcements: Announcement[] }) {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => go((current + 1) % announcements.length), 7000);
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl min-h-[200px] flex items-stretch" style={slideStyle}>
      {/* Grid overlay */}
      ...
      <div className={`relative z-10 flex flex-col justify-center px-8 md:px-14 py-10 flex-1 transition-all duration-200 ${fading ? "opacity-0 translate-y-1" : "opacity-100 translate-y-0"}`}>
        {slide ? (
          ...
          <p className="text-white/30 text-sm">{isTh ? "ยังไม่มีประกาศ" : "No announcements"}</p>
        )}
      </div>
